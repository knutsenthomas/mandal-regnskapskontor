      ...
      <header className="fixed top-0 left-0 w-full h-20 z-50">
        ...
        <div 
          className={`absolute inset-0 bg-white transition-opacity duration-300 ease-in-out ${
            isTransparent ? 'opacity-0' : 'opacity-100 shadow-sm'
          }`} 
        />
          ...
          <div className="flex items-center justify-between h-full">
            ...
            */}
            ...
            <nav className={`hidden lg:flex items-center gap-8 ml-auto ${isOpen ? 'opacity-0' : 'opacity-100'}`}>
              {['hjem', 'tjenester', 'om-oss'].map((item) => (
                <button 
                  key={item}
                  onClick={() => item === 'hjem' ? handleLogoClick() : handleLinkClick(item)} 
                  className={`font-bold text-sm tracking-widest uppercase whitespace-nowrap transition-colors duration-300 ${
                    isTransparent 
              ...
              <Button 
                ...
                className={`font-bold rounded-full whitespace-nowrap transition-all duration-300 ${
                  isTransparent 
            ...
            <div className={`lg:hidden ${isOpen ? 'hidden' : 'block'}`}>
              <button
                ...
                className={`flex items-center justify-center p-2 transition-colors duration-300 focus:outline-none ${
                   isTransparent ? 'text-white' : 'text-[#1B4965]'
                }`}
              >
                 <Menu size={32} />
              </button>
        ...
        {isOpen && (
          ...
          >
             ...
             <div className="flex-1 flex flex-col items-center justify-center space-y-8 p-4 w-full overflow-y-auto">
               {['hjem', 'tjenester', 'om-oss', 'kontakt'].map((link, index) => (
                 <motion.button 
                   key={link}
